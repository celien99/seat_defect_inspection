...
@dataclass(slots=True)
class _TransformerTokenBatch:
    """记录 transformer token 及其原始网格尺寸。"""
...
class _TorchTransformerPatchFeatureExtractor:
    """Transformer 特征提取器，用于 Transformer PatchCore 后端。"""
    ...
    def extract(
        ...
        *,
        ...
        """提取 Swin patch token embedding，并按 mask 筛出有效 token。"""
        feature_image = _prepare_feature_image(image, target_mask=target_mask)
        resized_image = cv2.resize(
            feature_image,
            (self.config.image_size, self.config.image_size),
        ...
        input_tensor = _prepare_torch_input(resized_image, self.config).to(self.device)
        with torch.inference_mode():
            token_batch = _extract_swin_patch_tokens(self.model, input_tensor)

        token_array = token_batch.tokens[0].detach().cpu().numpy().astype(np.float32)
        grid_rows, grid_cols = token_batch.grid_shape
...
def _load_torch_transformer_backbone(config: PatchCoreConfig) -> Any:
    """加载 Transformer PatchCore 使用的 torchvision Swin backbone。"""
    ...
    if config.backbone_weights_path:
        ...
        normalized_state_dict = _normalize_state_dict_keys(state_dict)
        missing_keys, unexpected_keys = model.load_state_dict(normalized_state_dict, strict=False)
        critical_missing = [key for key in missing_keys if not key.startswith("head.")]
        if critical_missing:
            raise RuntimeError(
    ...
    if int(config.image_size) != int(default_image_size):
        raise RuntimeError(
            "torchvision 预训练 Swin 权重要求 patchcore.image_size="
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "可选方案：1) 配置 patchcore.backbone_weights_path 指向本地权重；"
            "2) 先把 torchvision Swin 权重缓存到项目 .torch_cache。"
        ) from exc
...
def _resolve_transformer_builder(backbone_name: str) -> tuple[Any, Any, int]:
    ...
    normalized = backbone_name.strip().lower()
    builders = {
        "swin_t": (swin_t, Swin_T_Weights.DEFAULT if Swin_T_Weights is not None else None, 384),
    }
    builder = builders.get(normalized)
    if builder is None or builder[0] is None:
        supported = ", ".join(sorted(builders))
...
def _extract_swin_patch_tokens(model: Any, input_tensor: Any) -> _TransformerTokenBatch:
    """Return final Swin patch tokens without the classification head."""
    if not hasattr(model, "features"):
        raise ValueError("Transformer PatchCore 当前只支持 torchvision SwinTransformer 模型")
    feature_map = model.features(input_tensor)
    if hasattr(model, "norm"):
        feature_map = model.norm(feature_map)
    if getattr(feature_map, "ndim", None) != 4:
        raise ValueError("Transformer PatchCore 的 Swin 特征图必须是四维张量")
    batch_size, grid_rows, grid_cols, channels = feature_map.shape
    return _TransformerTokenBatch(
        tokens=feature_map.reshape(batch_size, grid_rows * grid_cols, channels),
